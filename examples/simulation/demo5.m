...
for i = 1:length(Rs)
    ...
    J1 = K*prod(R_true)/min(R_true); % First sketch dimension
    J2 = K*prod(R_true); % Second sketch dimension
    ...
    normY = tensornorm(Y);
    tucker_ts_error = tensornorm(Y - tensor(ttensor(G_ts, A_ts)))/normY
    tucker_ttmts_error = tensornorm(Y - tensor(ttensor(G_ttmts, A_ttmts)))/normY
    tucker_als_error = tensornorm(Y - tensor(Y_tucker_als))/normY
